...
class PostgresStorage:
    """
    This class is responsible for handling PostgreSQL operations with SSL support
    ...
    def create_tables(self):
        ...
        self.cursor.execute(create_drivers_table)
        self.cursor.execute(create_restaurants_table)
        self.conn.commit()
        print("Tables created successfully")

    def insert_drivers(self, json_data):
        """Insert drivers data into PostgreSQL with duplicate handling."""
        data = pd.read_json(StringIO(json_data.decode('utf-8')))

        insert_query = """
        INSERT INTO drivers (
            driver_id, uuid, first_name, last_name, date_birth,
            city, country, phone_number, license_number, vehicle_type,
            vehicle_make, vehicle_model, vehicle_year, vehicle_license_plate,
            cpf, dt_current_timestamp
        ) VALUES (
        ...
        data = data.drop_duplicates(subset=['driver_id'], keep='first')

        values = [
            (
                row.driver_id, row.uuid, row.first_name, row.last_name, row.date_birth,
                row.city, row.country, row.phone_number, row.license_number, row.vehicle_type,
                row.vehicle_make, row.vehicle_model, row.vehicle_year, row.vehicle_license_plate,
                row.cpf, row.dt_current_timestamp
            )
        ...
        self.conn.commit()
        print(f"Inserted {len(values)} records into PostgreSQL drivers table")

    def insert_restaurants(self, json_data):
        """Insert restaurants data into PostgreSQL."""
        data = pd.read_json(StringIO(json_data.decode('utf-8')))

        insert_query = """
        INSERT INTO restaurants (
            restaurant_id, uuid, name, address, city, country,
            phone_number, cuisine_type, opening_time, closing_time,
            average_rating, num_reviews, cnpj, dt_current_timestamp
        ) VALUES (
        ...
        values = [
            (
                row.restaurant_id, row.uuid, row.name, row.address,
                row.city, row.country, row.phone_number, row.cuisine_type,
                row.opening_time, row.closing_time, row.average_rating,
                row.num_reviews, row.cnpj, row.dt_current_timestamp
            )
            for row in data.itertuples(index=False)
        ]
        ...
        execute_batch(self.cursor, insert_query, values, page_size=100)
        self.conn.commit()
        print(f"Inserted {len(values)} records into PostgreSQL restaurants table")
